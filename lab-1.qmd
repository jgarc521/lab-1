...
mpg <- mpg |>
  mutate(fuel_type = if_else(fl %in% c("p", "r", "d"), "petrol", "alternative energy")); head(mpg)
```
...
mpg <- mpg |>
  mutate(
    trans_type = str_split(trans, "\\(", simplify = TRUE), # split string  at first parenthesis(as matrix)
    trans_type = trans_type[,2], # index to get trans type
    trans_type = str_replace(trans_type, "\\)", "") # remove extra parenthesis
  ) |>
  rename(trans_style = trans) # rename
